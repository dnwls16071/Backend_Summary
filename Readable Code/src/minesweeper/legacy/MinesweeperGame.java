...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int selectedColIndex;   // 사용자가 선택한 Col 인덱스
            switch (cellInputCol) {
                case 'a':
                    selectedColIndex = 0;
                    break;
            ...
            int selectedRowIndex = Character.getNumericValue(cellInputRow) - 1; // 사용자가 선택한 Row 인덱스
            if (userActionInput.equals("2")) {
                board[selectedRowIndex][selectedColIndex] = "⚑";
                boolean isAllOpened = true; // 모두 다 열었는가?
                for (int row = 0; row < 8; row++) {
                    for (int col = 0; col < 10; col++) {
